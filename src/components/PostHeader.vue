<template>
  <header>
    <div v-if="post.cover" class="post-cover overflow-hidden relative text-shadow" :class="[post.fullScreen ? 'fullScreen' : 'max-h-cover']">
      <div
        ...
        <div class="m-auto">
          ...
          <p class="text-white">
            <span v-if="post.author">
              <g-link
                :to="`${post.author.path}/`"
                class="text-white capitalize border-b border-transparent hover:border-white transition-colors duration-300"
                >{{ titleCase(post.author.title) }}</g-link
              >
              &bull;
            </span>
            <span class="text-yellow-200 mr-1">Last Updated</span>
    ...
    <div v-else class="pt-24">
      <div class="max-w-xl md:max-w-3xl xl:max-w-4xl mx-auto text-center px-6">
        ...
        <h1 class="text-3xl sm:text-5xl leading-tight font-sans font-bold mb-2 text-black text-shadow">
          {{ post.title }}
        </h1>
