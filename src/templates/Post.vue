<template>
  <Layout>
    <main>
      ...
      <article
        class="mx-auto px-6 sm:px-12 pt-16"
        :class="{
      ...
      >
        ...
        <div :class="{ 'pb-10': $page.post.author || $page.post.tags }" class="markdown text-xl leading-normal text-gray-700" v-html="$page.post.content" />

        <footer v-if="$page.post.author || $page.post.tags" class="max-w-xl md:max-w-2xl xl:max-w-3xl mx-auto flex flex-wrap pb-10 sm:pb-16">
          <div>
            <g-link
              v-for="tag in $page.post.tags"
              :key="tag.id"
              :to="`${tag.path}/`"
              class="inline-block text-teal-400 hover:text-white hover:bg-green-500 border border-teal-400 font-sans font-bold text-xs sm:text-sm px-4 py-2 mr-4 mb-2 rounded-full transition-colors duration-300"
            >
              <svg class="inline w-3 fill-current align-middle mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" role="img">
          ...
          >
            ...
            <div class="px-4 sm:w-4/5 text-center sm:text-left">
              ...
              <p class="leading-normal">
                <g-link :to="`${$page.post.author.path}/`" class="text-blue-500 hover:text-blue-400 transition-colors duration-300"
...
export default {
  ...
  computed: {
    ...
    ogImageUrl() {
      return this.$page.post.cover || `${this.$static.metadata.siteUrl}/images/bleda-card.png`;
    }
  }
};
</script>
